import { OrderStatusPill } from "@/components/orders/status-pill";
import type { RecentOrderRow } from "@/lib/dashboard/types";
import { formatCurrency } from "@/lib/format";

const dateFmt = new Intl.DateTimeFormat("en-US", {
  dateStyle: "medium",
  timeStyle: "short",
});

type Props = {
  orders: RecentOrderRow[];
};

export function RecentOrdersTable({ orders }: Props) {
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm">
      <div className="flex items-center justify-between border-b border-zinc-200 px-4 py-3">
        <h2 className="text-base font-semibold text-zinc-900">Recent orders</h2>
        <span className="text-xs text-zinc-400">Filtered by your selection</span>
      </div>
      {orders.length === 0 ? (
        <p className="p-6 text-sm text-zinc-500">No orders in this range.</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50 text-xs font-semibold uppercase tracking-wide text-zinc-500">
                <th className="px-4 py-2">Order</th>
                <th className="px-4 py-2">When</th>
                <th className="px-4 py-2">By</th>
                <th className="px-4 py-2 text-right">Total</th>
                <th className="px-4 py-2">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100">
              {orders.map((o) => (
                <tr key={o.id} className="hover:bg-zinc-50/60">
                  <td className="px-4 py-2 font-mono text-xs font-medium text-zinc-900">
                    {o.orderNumber}
                  </td>
                  <td className="px-4 py-2 text-zinc-600">{dateFmt.format(new Date(o.createdAt))}</td>
                  <td className="max-w-[140px] truncate px-4 py-2 text-zinc-600" title={o.createdByLabel}>
                    {o.createdByLabel}
                  </td>
                  <td className="px-4 py-2 text-right font-medium tabular-nums text-zinc-900">
                    {formatCurrency(o.totalAmount)}
                  </td>
                  <td className="px-4 py-2">
                    <OrderStatusPill status={o.orderStatus} />
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}
